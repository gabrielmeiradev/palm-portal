...
const getFileNameToDeploy = async () => {
    ...
    if (!fileName) {
        const filesInDir = fs.readdirSync(process.cwd()).filter((file) => { return file.endsWith(".zip") });
        ...
        socket.connect(SERVER_PORT, SERVER_HOST, function() {
            ...
            reader.on("readable", function () {
                ...
                while (data = this.read()) {
                    ...
                    spinner.update(`Uploading file: ${((reader.bytesRead / (1024 * 1024)).toFixed(2))} MB of ${(totalSize / (1024 * 1024)).toFixed(2)} MB in ${time} seconds`)
                }
            })
